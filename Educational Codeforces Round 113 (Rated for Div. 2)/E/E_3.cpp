...
int step = 0;
void solve(int cur, int sum, int rem) {
    step++;
    de(0) printf("solve(%lld, %lld)\n", cur, sum);
    if (cur >= k) {
        if (l[cur] <= rem && rem <= r[cur]) {
            keep.PB(rem);
            sum = (sum + cht[cur][rem]) % MOD;
            if (sum == h) {
                cand.PB(keep);
            }
            keep.PP();
    ...
    for (int i=l[cur]; i<=r[cur]; i++) {
        keep.PB(i);
        solve(cur+1, (sum + cht[cur][i]) % MOD, rem - i);
        keep.PP();
    }
}
...
int32_t main() {
    ...
    solve(0, 0, (1 + (1<<k)) * (1<<k) / 2);
    de(1) printf("%lld steps used\n", step);
