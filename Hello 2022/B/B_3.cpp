    ...
    while (t--) {
        ...
        for (int i=1; i<=n; i++) {
            ...
            if (!mini_idx && !maxi_idx && mini_idx != maxi_idx &&
                l[i] == l[mini_idx] && r[i] == r[maxi_idx] && c[i] < c[mini_idx] + c[maxi_idx]) {
                mini_idx = i;
                maxi_idx = i;
            }

            // cerr << "mini_idx = " << mini_idx << ", maxi_idx = " << maxi_idx << "\n";
            // cerr << "ans = ";
            if (mini_idx == maxi_idx)
                printf("%lld\n", c[mini_idx]);
            else {
                if (r[mini_idx] == r[maxi_idx])
                    printf("%lld\n", c[mini_idx]);
                else if (l[mini_idx] == l[maxi_idx])
                    printf("%lld\n", c[maxi_idx]);
                else
                    printf("%lld\n", c[mini_idx] + c[maxi_idx]);
            }
        }
    }
